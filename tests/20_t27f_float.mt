...
fn main() {
    ...
    test::assert(t27f::mantissa(x) == 100, "mantissa of 100 is 100");
    test::assert(t27f::exponent(x) == 0,   "exponent of 100×3^0 is 0");
    print("PASS from_parts / mantissa / exponent");

    // --- Zero properties ---
    let z = t27f::ZERO;
    test::assert(t27f::is_zero(z) == true, "ZERO is zero");
    test::assert(t27f::mantissa(z) == 0,   "ZERO mantissa is 0");
    // No dual zero: there is exactly one zero representation
    let z2 = t27f::from_parts(5, 0);
    test::assert(t27f::is_zero(t27f::normalize(z2)) == true, "0×3^5 normalizes to ZERO");
    print("PASS no dual zero");

    // --- Negation is exact trit-flip ---
    let pos = t27f::from_parts(0, 42);
    let neg_val = t27f::neg(pos);
    test::assert(t27f::mantissa(neg_val) == -42, "neg(42) mantissa = -42");
    test::assert(t27f::exponent(neg_val) == 0,   "neg preserves exponent");
    // Double negation is identity
    let pos2 = t27f::neg(neg_val);
    test::assert(t27f::mantissa(pos2) == 42, "neg(neg(42)) = 42");
    ...
    test::assert(t27f::to_int(sum) == 300, "100 + 200 = 300");
    print("PASS addition");

    // Subtraction
    let diff = t27f::sub(b, a);
    test::assert(t27f::to_int(diff) == 100, "200 - 100 = 100");
    ...
    // 3 × 9 = 27 = 1 × 3^3
    let prod_val = t27f::to_int(prod);
    test::assert(prod_val == 27, "3 × 9 = 27");
    print("PASS multiplication");

    // --- Comparison ---
    test::assert(t27f::compare(b, a) == +1, "200 > 100");
    test::assert(t27f::compare(a, b) == -1, "100 < 200");
    test::assert(t27f::compare(a, a) == 0,  "100 == 100");
    print("PASS comparison");

    // --- Absolute value ---
    let neg_val2 = t27f::from_parts(0, -50);
    let abs_val = t27f::abs(neg_val2);
    test::assert(t27f::mantissa(abs_val) > 0, "abs(-50) is positive");
